...
public class Inventory {
	...
	private int index; // # of items equipped
	...
	public void draw(Graphics2D g) {
		...
		// Had to move inventory up to see it on linux - remove the constants added to y
		// values to return inventory to previous position
		...
		if (SaveManager.getInstance().getHealthPotionNum() > 0) {
			...
			g.setFont(dogicaPixelBold.deriveFont(Font.PLAIN, 17F));
			g.setColor(Color.white);
			g.drawString(SaveManager.getInstance().getHealthPotionNum() + "", (3 * 60) + 537, 529 + 90);
		}
